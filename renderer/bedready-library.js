...
(function () {
  ...
  var root = null, body = null, items = [], linkedListenerBound = false;
  ...
  function build() {
    ...
    root.innerHTML =
      '<div class="brl-modal" role="dialog" aria-modal="true" aria-label="BedReady library" style="width:100%;max-width:560px;max-height:82vh;overflow:auto;border-radius:18px;background:var(--surface,#0f172a);color:var(--fg,#e2e8f0);border:1px solid var(--line,rgba(255,255,255,.1));box-shadow:0 20px 60px rgba(0,0,0,.5);">' +
        '<div style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:18px 20px;border-bottom:1px solid var(--line,rgba(255,255,255,.1));">' +
  ...
  function btn(label, kind) {
    var grad = 'background:linear-gradient(100deg,#7c3aed,#06b6d4 55%,#84cc16);color:#fff;';
    var plain = 'background:var(--surface-2,#1e293b);color:var(--fg,#e2e8f0);border:1px solid var(--line,rgba(255,255,255,.12));';
  ...
  function renderConnecting() {
    setBody(
      ...
      '<p style="margin:0 0 16px;color:var(--fg-muted,#94a3b8);font-size:13px;">Sign in there and click <b>Connect</b> — this panel updates automatically once it links.</p>' +
      btn('Recheck', 'plain'));
    // "Recheck" reuses the plain handler → refresh
    body.querySelector('[data-act="plain"]') && body.querySelector('[data-act="plain"]').addEventListener('click', refresh);
  }
  ...
  function renderNotLinked() {
    setBody(
      ...
      '<p style="margin:0 0 18px;color:var(--fg-muted,#94a3b8);font-size:13px;">Opens bedready.io in your browser to sign in. Nothing is uploaded — this only downloads your own saves.</p>' +
    ...
    if (items.length) {
      list = '<div style="margin:14px 0;display:grid;gap:8px;">' + items.map(function (it) {
        var cover = it.cover ? '<img src="' + esc(it.cover) + '" alt="" style="width:44px;height:44px;object-fit:cover;border-radius:8px;flex:0 0 auto;">'
          : '<div style="width:44px;height:44px;border-radius:8px;background:var(--surface-3,#334155);flex:0 0 auto;"></div>';
        var meta = it.downloadUrl ? '' : '<span style="font-size:11px;color:var(--fg-muted,#94a3b8);"> · link only</span>';
        return '<div style="display:flex;align-items:center;gap:10px;padding:8px;border:1px solid var(--line,rgba(255,255,255,.1));border-radius:10px;">' +
          ...
          '<div style="font-size:11px;color:var(--fg-muted,#94a3b8);">' + esc((it.fileType || '3mf')).toUpperCase() + meta + '</div></div></div>';
      }).join('') + '</div>';
    }
    var downloadable = items.filter(function (i) { return i.downloadUrl; }).length;
    setBody(
      '<div style="display:flex;align-items:center;justify-content:space-between;gap:8px;">' +
        '<span style="color:var(--fg-muted,#94a3b8);font-size:13px;">' + (items.length ? (items.length + ' saved design' + (items.length === 1 ? '' : 's')) : 'Connected — sync to pull your saves.') + '</span>' +
        btn('Sync', 'sync') +
      '</div>' + list +
      ...
      '</div>' +
      '<div class="brl-result" style="margin-top:12px;font-size:13px;"></div>');
  }

  function result(html, color) {
    var el = body && body.querySelector('.brl-result');
    if (el) el.innerHTML = '<span style="color:' + (color || 'var(--fg-muted,#94a3b8)') + ';">' + html + '</span>';
  }
  ...
  async function refresh() {
    setBody('<p style="color:var(--fg-muted,#94a3b8);">Checking…</p>');
    try {
