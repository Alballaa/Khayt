...
- **Beta / RC:** the `3.7.0` line is open and has moved to a CANDIDATE — **`v3.7.0-beta.14` is being cut** (2026-08-30), for all three platforms, and it replaces `beta.13` as the promotion candidate. Latest *published* pre-release is **v3.7.0-beta.13** (2026-08-29). **A candidate is named `-beta.N` here, and the candidacy lives in docs/RELEASE-HOLD.md rather than in the version string** — see the channel warning below. The line has run `beta.1` (delta cloud writes), `beta.2` (a print library that can outgrow its disk), `beta.3` (a macOS build of `beta.2`), `beta.4` (storage-provider signup links), `beta.5` (printers and models, first all-platform cut on this line), `beta.6` and `beta.9` (the vendor audits), `beta.7` (Medusa and both kinds of Duet), `beta.8` (installable designs and cost per model), `beta.10` (the audit method's last three surfaces), `beta.11` and `beta.12` (the verification-code dialog, which could not run) `beta.13` (the cloud sign-in freeze that stopped it running, and the review around it) and `beta.14` (the nozzle counter that had never counted, its sourced replacement table, the printer facts sweep and the catalogue). The `3.6.0` line closed when `v3.6.0-rc.4` was promoted to stable on 2026-08-21; it ran `beta.1`–`beta.19` then `rc.1`–`rc.4`.
