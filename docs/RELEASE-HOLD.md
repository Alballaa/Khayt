...
| **Beta / RC** | **v3.7.0-beta.17** (2026-08-31, being cut — **this is the promotion candidate**, replacing `beta.16`, which is superseded rather than withdrawn and remains the newest published until this one lands, replacing `beta.15`; named `-beta.N` because an `-rc` is invisible to existing beta installs, see VERSIONING.md). It carries the content-language work and the **eighteen readers that had been treating a shop's own text as an English-or-Arabic pair** — two of which put a blank name into messages sent to customers, two of which submitted ZATCA e-invoices with no seller street, and one of which (the server's catalogue whitelist, silently dropping every new field) made the storefront feature inert in production. Newest published: `v3.7.0-beta.16` — this same release. The previous candidate `v3.7.0-beta.15` is superseded rather than withdrawn; **do not recommend it, or anything earlier, to a shop that does not write English or Arabic**. |
