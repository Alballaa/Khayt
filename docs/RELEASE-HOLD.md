...
| **Beta / RC** | **v3.7.0-beta.15** (2026-08-30, being cut — **this is the promotion candidate**, replacing `beta.14`; it is named `-beta.N` because an `-rc` is invisible to existing beta installs, see VERSIONING.md) — newest published: `v3.7.0-beta.14`, **which must not be recommended**: its catalogue cannot accept a photo, and if it could it would collapse them all to one file. beta.15 fixes both, puts the nozzle warning on a dashboard for the first time, and repairs Meridian's attention bar, which had never worked. |
