...
| **Beta / RC** | **v3.7.0-beta.5** (2026-08-24) | **The 3.7.0 line is open and has run five cuts.** `beta.1` flipped `DELTA_WRITES` — cloud sync writes only what changed, the first real exercise of that path against production. `beta.2` is the substance of the line: the print library can outgrow its disk, because cold models *move* to a bucket rather than being copied to one, with nothing deleted locally until the bucket has answered a separate request with an etag matching the local content. It also brought the fifteen-provider endpoint dropdown and Google Drive as a backend. `beta.3` carries **no app change at all** — `beta.2` went out Windows + Linux only because `BUILD_MAC` was unset at tag time, so mac users on the channel had nothing to move to; `beta.3` is `beta.2`'s tree rebuilt with the variable set. `beta.4` carries the storage-provider signup links ([#722](https://github.com/KhaytApp/Khayt/pull/722)) and was tagged from `11ef165` ([#723](https://github.com/KhaytApp/Khayt/pull/723)), published on 2026-08-23 — **but only after sitting bumped on `main` with no tag for most of that day**, during which release CI had never run, no installer existed, and a beta user's newest available build was `beta.3` while `package.json` said `beta.4`. Like `beta.2` it went out **Windows + Linux only** (`BUILD_MAC` unset — deliberately this time, it being a small UI change and not a promotion candidate). **`beta.5` (2026-08-24) is the current cut and the first on this line built for all three platforms**, tagged from `cdeefae` ([#741](https://github.com/KhaytApp/Khayt/pull/741)): the printer-and-model release — a printer that changed address now says so instead of reading as one that had gone away, Khayt says what is likely to go *wrong* with a model rather than only what it costs, kits can be filed after the fact from a picker, and a camera Khayt merely guessed at is no longer switched on before anything has answered. `BUILD_MAC` was set before the tag and **deleted after** — it is sticky, and the run summary says which way it went; `carry-mac-manifest` correctly skipped, a real mac build having shipped. **Three commits sit on `main` untagged behind it** — [#742](https://github.com/KhaytApp/Khayt/pull/742), [#743](https://github.com/KhaytApp/Khayt/pull/743) and [#745](https://github.com/KhaytApp/Khayt/pull/745) — so `beta.5` is already not what `main` is. The 3.6.0 candidate line is closed: `v3.6.0-rc.4` became stable v3.6.0 with no code change between the two. rc.1, rc.2 and rc.3 were each *replaced* rather than promoted, because a candidate is meant to *be* what stable will be and `main` had overtaken each one while it sat. The lesson to carry into this line: decide replace-vs-promote *on the day* with `git rev-list --count <tag>..origin/main` and an empty `[Unreleased]`, not from this table. **A version in `package.json` is evidence the cut landed, never that it shipped — confirm with `gh release list` or `git ls-remote --tags`; the `beta.4` day above is what that trap looks like in practice.** |

Last verified 2026-08-25 against `gh release list` and `git ls-remote --tags origin`. These rot fast — confirm with
