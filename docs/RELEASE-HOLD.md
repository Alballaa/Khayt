...
| **Beta / RC** | **v3.7.0-beta.3** (2026-08-23) | **The 3.7.0 line is open and has run three cuts.** `beta.1` flipped `DELTA_WRITES` — cloud sync writes only what changed, the first real exercise of that path against production. `beta.2` is the substance of the line: the print library can outgrow its disk, because cold models *move* to a bucket rather than being copied to one, with nothing deleted locally until the bucket has answered a separate request with an etag matching the local content. It also brought the fifteen-provider endpoint dropdown and Google Drive as a backend. `beta.3` carries **no app change at all** — `beta.2` went out Windows + Linux only because `BUILD_MAC` was unset at tag time, so mac users on the channel had nothing to move to; `beta.3` is `beta.2`'s tree rebuilt with the variable set. **⚠ `3.7.0-beta.4` is bumped on `main` ([#723](https://github.com/KhaytApp/Khayt/pull/723), `11ef165`) and was never tagged** — release CI never ran, no installer exists, and a beta user's newest available build is `beta.3`. It would carry the storage-provider signup links ([#722](https://github.com/KhaytApp/Khayt/pull/722)). Finish it by tagging `11ef165` and pushing the tag, or the version in `package.json` will keep claiming a release that does not exist. **A version in `package.json` is evidence the cut landed, never that it shipped — confirm with `gh release list` or `git ls-remote --tags`; this row is what that trap looks like in practice.** The 3.6.0 candidate line is closed: `v3.6.0-rc.4` became stable v3.6.0 with no code change between the two. rc.1, rc.2 and rc.3 were each *replaced* rather than promoted, because a candidate is meant to *be* what stable will be and `main` had overtaken each one while it sat. The lesson to carry into this line: decide replace-vs-promote *on the day* with `git rev-list --count <tag>..origin/main` and an empty `[Unreleased]`, not from this table |
