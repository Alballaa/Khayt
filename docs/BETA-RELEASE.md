...
| **Beta / RC** | `v3.7.0-beta.15` (`v3.7.0-beta.16` being cut — the promotion candidate; **do not recommend anything before `beta.16` to a shop that does not write English or Arabic**: its own name, its clients' names and its ZATCA seller address came out blank) | [Pre-releases](https://github.com/khaytapp/Khayt/releases) (filter *Pre-release*) | Opt-in via Settings |
