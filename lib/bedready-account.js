...
function write(userDataDir, data) {
  try {
    ...
    // break re-auth). rename() is atomic within the same directory.
...
function parseDeepLink(url) {
  ...
  return { access: p.get('access_token') || '', refresh, expires: normExpiresSeconds(exp) };
}
...
async function getAccessToken(userDataDir, now = Math.floor(Date.now() / 1000)) {
  ...
  write(userDataDir, { access: j.access_token, refresh: j.refresh_token || d.refresh, expires });
