 ...
 *
 * FALLBACK — remove in a later release. A desktop update and a cloud deploy are
 * separate events, so this build can meet a server that has not shipped #10 yet;
 * there the new path is simply an unmatched route and the server answers 404. We
 * retry the old public path in that case rather than showing an empty thread.
 * Once the cloud is deployed everywhere, drop the fallback — while it is here, a
 * 404 that really means "not this shop's token" also falls through to the public
 * read, which is exactly the exposure being closed.
 */
async function portalMessages(baseUrl, shopId, token, authToken) {
  ...
  if (r.status !== 404) throw new Error((r.body && r.body.error) || `HTTP ${r.status}`);

  const legacy = await httpTransport(baseUrl)({ method: 'GET', path: `/v1/p/${seg(token)}/messages` });
  if (legacy.status !== 200) throw new Error((legacy.body && legacy.body.error) || `HTTP ${legacy.status}`);
  return (legacy.body && legacy.body.messages) || [];
}
