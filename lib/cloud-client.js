...
async function inviteMember(baseUrl, shopId, token, email, role) {
  const r = await httpTransport(baseUrl, token)({ method: 'POST', path: `/v1/shops/${shopId}/members/invite`, body: { email, role } });
...
async function listMembers(baseUrl, shopId, token) {
  const r = await httpTransport(baseUrl, token)({ method: 'GET', path: `/v1/shops/${shopId}/members` });
  if (r.status !== 200) throw new Error((r.body && r.body.error) || `HTTP ${r.status}`);
...
async function removeMember(baseUrl, shopId, token, email) {
  const r = await httpTransport(baseUrl, token)({ method: 'DELETE', path: `/v1/shops/${shopId}/members/${encodeURIComponent(email)}` });
  if (r.status !== 200) throw new Error((r.body && r.body.error) || `HTTP ${r.status}`);
  return { ok: true };
}
...
async function putCatalog(baseUrl, shopId, token, payload) {
  const r = await httpTransport(baseUrl, token)({ method: 'PUT', path: `/v1/shops/${shopId}/catalog`, body: { catalog: payload } });
  if (r.status !== 200) throw new Error((r.body && r.body.error) || `HTTP ${r.status}`);
...
async function getCatalog(baseUrl, shopId, token) {
  const r = await httpTransport(baseUrl, token)({ method: 'GET', path: `/v1/shops/${shopId}/catalog` });
...
async function getReviewSummary(baseUrl, shopId) {
  const r = await httpTransport(baseUrl, null)({ method: 'GET', path: `/v1/shops/${shopId}/reviews` });
...
async function publishPortal(baseUrl, shopId, token, pubToken, kind, payload, customerEmail) {
  ...
  const r = await httpTransport(baseUrl, token)({
    method: 'PUT', path: `/v1/shops/${shopId}/published/${pubToken}`, body,
  });
  if (r.status !== 200) throw new Error((r.body && r.body.error) || `HTTP ${r.status}`);
  return { ok: true };
}
...
async function unpublishPortal(baseUrl, shopId, token, pubToken) {
  const r = await httpTransport(baseUrl, token)({ method: 'DELETE', path: `/v1/shops/${shopId}/published/${pubToken}` });
  if (r.status !== 200) throw new Error((r.body && r.body.error) || `HTTP ${r.status}`);
  return { ok: true };
}
...
async function listPublished(baseUrl, shopId, token) {
  const r = await httpTransport(baseUrl, token)({ method: 'GET', path: `/v1/shops/${shopId}/published` });
  if (r.status !== 200) throw new Error((r.body && r.body.error) || `HTTP ${r.status}`);
...
async function listIntake(baseUrl, shopId, token) {
  const r = await httpTransport(baseUrl, token)({ method: 'GET', path: `/v1/shops/${shopId}/intake` });
  if (r.status !== 200) throw new Error((r.body && r.body.error) || `HTTP ${r.status}`);
  return (r.body && r.body.items) || [];
}
...
async function deleteIntake(baseUrl, shopId, token, id) {
  const r = await httpTransport(baseUrl, token)({ method: 'DELETE', path: `/v1/shops/${shopId}/intake/${id}` });
  if (r.status !== 200) throw new Error((r.body && r.body.error) || `HTTP ${r.status}`);
  return { ok: true };
}
...
async function portalMessages(baseUrl, token) {
  const r = await httpTransport(baseUrl)({ method: 'GET', path: `/v1/p/${token}/messages` });
  if (r.status !== 200) throw new Error((r.body && r.body.error) || `HTTP ${r.status}`);
...
async function portalReply(baseUrl, shopId, token, authToken, text) {
  const r = await httpTransport(baseUrl, authToken)({ method: 'POST', path: `/v1/shops/${shopId}/published/${token}/message`, body: { text } });
  if (r.status !== 200) throw new Error((r.body && r.body.error) || `HTTP ${r.status}`);
  return r.body || {};
}
...
async function storefrontStats(baseUrl, shopId, token) {
  const r = await httpTransport(baseUrl, token)({ method: 'GET', path: `/v1/shops/${shopId}/sfstats` });
  if (r.status !== 200) throw new Error((r.body && r.body.error) || `HTTP ${r.status}`);
