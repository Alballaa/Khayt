...
/**
 * How many facets of a 3MF's mesh can be built before the answer costs more
 * than it is worth. See from3mf step 3 for the measurements behind it.
 */
const MAX_3MF_FACETS = 4000000;
 ...
 */
function from3mf(buf, out, riskOpts) {
  let members;
  ...
  if (facets > MAX_3MF_FACETS) {
    out.warnings.push('mesh-too-large');
    return out;
  }
  let tris = null;
...
function intake(input, opts) {
  ...
  try {
    ...
    if (kind === '3mf') return from3mf(buf, out, riskOpts);
