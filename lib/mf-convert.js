...
(function (global) {
   ...
   * object bounding box. Used for the "will it fit the target bed?" check. Best-effort: returns
   * null if there's no readable geometry, the mesh is too large to scan quickly, or objects are
   * component-only (no direct vertices). Advisory, never blocks a conversion.
   */
  function computeBounds(members) {
    ...
    while ((om = objRe.exec(text))) {
      ...
      while ((vm = vRe.exec(om[2]))) { has = true; const x = +vm[1], y = +vm[2], z = +vm[3]; if (x < mnx) mnx = x; if (y < mny) mny = y; if (z < mnz) mnz = z; if (x > mxx) mxx = x; if (y > mxy) mxy = y; if (z > mxz) mxz = z; }
      if (has) objs[om[1]] = { mnx, mny, mnz, mxx, mxy, mxz };
    }
    if (!Object.keys(objs).length) return null;
    let Gx0 = Infinity, Gy0 = Infinity, Gz0 = Infinity, Gx1 = -Infinity, Gy1 = -Infinity, Gz1 = -Infinity, any = false;
    const acc = (x, y, z) => { any = true; if (x < Gx0) Gx0 = x; if (y < Gy0) Gy0 = y; if (z < Gz0) Gz0 = z; if (x > Gx1) Gx1 = x; if (y > Gy1) Gy1 = y; if (z > Gz1) Gz1 = z; };
    const corners = (b, t) => {
      for (const X of [b.mnx, b.mxx]) for (const Y of [b.mny, b.mxy]) for (const Z of [b.mnz, b.mxz]) {
        if (t) acc(X * t[0] + Y * t[3] + Z * t[6] + t[9], X * t[1] + Y * t[4] + Z * t[7] + t[10], X * t[2] + Y * t[5] + Z * t[8] + t[11]);
        else acc(X, Y, Z);
      }
    };
    ...
    if (items.length) {
      for (const it of items) {
        const b = objs[(/objectid="([^"]+)"/i.exec(it) || [])[1]]; if (!b) continue;
        const tm = (/transform="([^"]+)"/i.exec(it) || [])[1];
        const t = tm ? tm.trim().split(/\s+/).map(Number) : null;
        corners(b, (t && t.length === 12 && t.every(Number.isFinite)) ? t : null);
      }
    } else {
      for (const id in objs) corners(objs[id], null);
    }
    if (!any) return null;
    return { x: Math.round((Gx1 - Gx0) * 10) / 10, y: Math.round((Gy1 - Gy0) * 10) / 10, z: Math.round((Gz1 - Gz0) * 10) / 10 };
  }
  ...
  const api = { analyze, convert, readMembers, detectFlavour, extractFilaments, extractMeta, computeBounds, fitWarnings };
