...
(function (global) {
  ...
  function _extractCore(members, wantPaint) {
    const norm = (p) => String(p || '').replace(/^\/+/, '').toLowerCase();
    const modelMembers = members.filter((m) => /\.model$/i.test(m.name) && m.data && m.data.length <= 160 * 1024 * 1024);
    ...
    const soup = [], paintOut = [];
    const emit = (tri, code, t) => { soup.push(tri.map((p) => { const q = applyT(p, t); return [q[0] * scale, q[1] * scale, q[2] * scale]; })); if (wantPaint) paintOut.push(code); };
    if (items.length) {
      for (const it of items) {
        ...
        for (let i = 0; i < r.geo.length; i++) emit(r.geo[i], wantPaint ? r.pnt[i] : null, t);
      }
    } else {
      for (const k in objs) { const r = resolve(k, null); if (r) for (let i = 0; i < r.geo.length; i++) emit(r.geo[i], wantPaint ? r.pnt[i] : null, null); }
    }
    ...
    if (!soup.length) {
      for (const k in objs) {
        ...
        for (let i = 0; i < o.tris.length; i++) {
          const t = o.tris[i], a = o.verts[t[0]], b = o.verts[t[1]], c = o.verts[t[2]];
          if (a && b && c) emit([a, b, c], wantPaint && o.paint ? o.paint[i] : null, null);
        }
      }
    }
    if (!soup.length) return null;
    return wantPaint ? { triangles: soup, paint: paintOut } : soup;
  }

  function extractTriangles(members) { return _extractCore(members, false); }
  // Like extractTriangles but returns { triangles, paint } — paint[i] is the per-facet
  // paint code (or null) aligned to triangles[i]. Powers the true-multicolour preview.
  function extractTrianglesWithPaint(members) { return _extractCore(members, true); }
  ...
  const api = { analyze, convert, readMembers, detectFlavour, extractFilaments, extractMeta, computeBounds, extractTriangles, extractTrianglesWithPaint, fitWarnings };
