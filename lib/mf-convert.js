...
(function (global) {
  ...
  /**
   * Make the converted file's PRINTER + PROCESS settings genuinely U1-native by overlaying the resolved
   * Snapmaker U1 machine profile (real start/end/tool-change G-code, klipper flavour, build volume, tool
   * clearances) and a U1 process preset (layer height, walls, speeds…) read from the maker's INSTALLED
   * Snapmaker Orca. This is what stops Orca's "please confirm this G-code is safe" prompt — the embedded
   * G-code is then the U1's own, not the source Bambu machine's. Filament keys are left to
   * applyOrcaFilaments; Full Spectrum keys are applied afterwards so they win. No-op when the DB is
   * absent (falls back to the plain reprofile). Only meaningful for the Snapmaker U1 target.
   */
  function applyU1Native(obj, opts, target, report) {
    if (!orcaDb || target.id !== 'snapmaker-u1') return;
    const machine = orcaDb.u1MachineSettings();
    if (!machine || !Object.keys(machine).length) return; // Orca not installed → keep plain reprofile
    const procName = (opts && opts.process) || orcaDb.defaultU1Process();
    const proc = orcaDb.u1ProcessSettings(procName);
      ...
      out = out.map((m) => {
        ...
        if (/project_settings\.config$/i.test(m.name) || /model_settings\.config$/i.test(m.name)) {
          ...
          if (obj) {
            ...
            // Overlay the real U1 machine + process settings (from the installed slicer) so the printer
            // G-code and print settings are U1-native — runs before Full Spectrum so mix keys win.
            if (/project_settings\.config$/i.test(m.name) && target.flavour === 'orca') applyU1Native(obj, opts, target, report);
