...
(function (global) {
  ...
  function computeBounds(members) {
    ...
    while ((om = objRe.exec(text))) {
      ...
      const vRe = /<vertex\b[^>]*\bx="(-?[\d.eE+]+)"[^>]*\by="(-?[\d.eE+]+)"[^>]*\bz="(-?[\d.eE+]+)"/g; let vm;
      let mnx = Infinity, mny = Infinity, mnz = Infinity, mxx = -Infinity, mxy = -Infinity, mxz = -Infinity, has = false;
      while ((vm = vRe.exec(block))) { has = true; const x = +vm[1], y = +vm[2], z = +vm[3]; if (x < mnx) mnx = x; if (y < mny) mny = y; if (z < mnz) mnz = z; if (x > mxx) mxx = x; if (y > mxy) mxy = y; if (z > mxz) mxz = z; }
   ...
   */
  function extractTriangles(members) {
    // Parse EVERY .model member, not just the first — Bambu/Orca "split" 3MFs keep the
    // geometry in 3D/Objects/*.model and reference it from the root via p:path, so a
    // single-file read finds no mesh. Objects are keyed by "<normpath>#<id>" to avoid the
    // per-file id collisions, and components / build items follow their p:path.
    ...
    for (const mm of modelMembers) {
      ...
      if (!rootKey && /<build\b[^>]*>[\s\S]*?<item\b/i.test(text)) { rootKey = fkey; scale = fscale; }
      ...
      while ((om = objRe.exec(text))) {
        ...
        const verts = [];
        const vRe = /<vertex\b[^>]*\bx="(-?[\d.eE+]+)"[^>]*\by="(-?[\d.eE+]+)"[^>]*\bz="(-?[\d.eE+]+)"/g; let vm;
        while ((vm = vRe.exec(block))) verts.push([+vm[1], +vm[2], +vm[3]]);
        const tris = [];
        const tRe = /<triangle\b[^>]*\bv1="(\d+)"[^>]*\bv2="(\d+)"[^>]*\bv3="(\d+)"/g; let tm;
        while ((tm = tRe.exec(block))) tris.push([+tm[1], +tm[2], +tm[3]]);
        const comps = [];
        ...
        objs[fkey + '#' + id] = { verts, tris, comps };
      }
    }
    if (!Object.keys(objs).length) return null;
    if (!rootKey) { rootKey = norm(modelMembers[0].name); scale = UNIT_MM[((/<model\b[^>]*\bunit="([^"]+)"/i.exec(fileText[rootKey] || '') || [])[1] || 'millimeter').toLowerCase()] || 1; }
    ...
    function resolve(key, seen) {
      ...
      const out = [];
      for (const t of o.tris) { const a = o.verts[t[0]], b = o.verts[t[1]], c = o.verts[t[2]]; if (a && b && c) out.push([a, b, c]); }
      for (const cp of o.comps) {
        const child = resolve(cp.path + '#' + cp.ref, new Set(seen || []).add(key));
        if (child) for (const tri of child) out.push(tri.map((p) => applyT(p, cp.t)));
      }
      return (memo[key] = out);
    }

    const buildBlock = (/<build\b[^>]*>([\s\S]*?)<\/build>/i.exec(fileText[rootKey] || '') || [])[1];
    const items = [];
    if (buildBlock) { const itRe = /<item\b([^>]*?)\/?>/g; let im; while ((im = itRe.exec(buildBlock))) items.push(im[1]); }
    const soup = [];
    const emit = (tri, t) => soup.push(tri.map((p) => { const q = applyT(p, t); return [q[0] * scale, q[1] * scale, q[2] * scale]; }));
    if (items.length) {
      for (const it of items) {
        const oid = (/\bobjectid="([^"]+)"/i.exec(it) || [])[1];
        if (!oid) continue;
        const pp = (/\b(?:p:)?path="([^"]+)"/i.exec(it) || [])[1];
        const tris = resolve((pp ? norm(pp) : rootKey) + '#' + oid, null);
        if (!tris) continue; // skip an unresolved item rather than drop the whole mesh
        const t = parseT((/transform="([^"]+)"/i.exec(it) || [])[1]);
        for (const tri of tris) emit(tri, t);
      }
    } else {
      for (const k in objs) { const tris = resolve(k, null); if (tris) for (const tri of tris) emit(tri, null); }
    }
    return soup.length ? soup : null;
  }
  ...
  function convert(buf, opts = {}) {
    ...
    try {
      ...
      const coloursOk = mode === 'normalize' ? true : back.colorCount === filaments.length;
  ...
  const api = { analyze, convert, readMembers, detectFlavour, extractFilaments, extractMeta, computeBounds, extractTriangles, fitWarnings };
