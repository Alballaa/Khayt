...
(function (global) {
   ...
   *
   * The budget is unchanged and the zip-bomb defence is exactly as strong. All
   * that moves is who has first claim on it: the few kilobytes that say what the
   * file IS, before the megabytes that say what shape it is.
   */
  function readMembers(buf) {
    ...
    const take = (wantMesh) => {
      for (const e of zip.entries) {
        if (out.length >= MEMBERS_MAX_COUNT) return;
        if (MESH_MEMBER.test(e.name) !== wantMesh) continue;
        const data = zip.file(e.name);
        if (!data) continue;
        // Checked BEFORE counting: the original added a member's bytes to the
        // running total and then discarded it, so one oversized member could
        // close the door on every small one behind it.
        if (total + data.length > MEMBERS_TOTAL_BUDGET) return;
        total += data.length;
