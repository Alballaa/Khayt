...
(function (global) {
  ...
  function extractTriangles(members) {
    ...
    if (!model || model.length > 48 * 1024 * 1024) return null;
    const text = model.toString('utf8');
    const scale = UNIT_MM[((/<model\b[^>]*\bunit="([^"]+)"/i.exec(text) || [])[1] || 'millimeter').toLowerCase()] || 1;
    const objs = {};
    const objRe = /<object\b[^>]*\bid="([^"]+)"[^>]*>([\s\S]*?)<\/object>/g; let om;
    while ((om = objRe.exec(text))) {
      const block = om[2];
    ...
    function resolve(id, seen) {
      ...
      for (const cp of o.comps) {
        const child = resolve(cp.ref, new Set(seen || []).add(id));
        if (!child) return null;
        for (const tri of child) out.push(tri.map((p) => applyT(p, cp.t)));
      }
      return (memo[id] = out);
    }
    const buildBlock = (/<build\b[^>]*>([\s\S]*?)<\/build>/i.exec(text) || [])[1];
    ...
    if (items.length) {
      for (const it of items) {
        const tris = resolve((/objectid="([^"]+)"/i.exec(it) || [])[1], null);
        if (!tris) return null;
        const t = parseT((/transform="([^"]+)"/i.exec(it) || [])[1]);
        for (const tri of tris) emit(tri, t);
      }
    } else {
      for (const id in objs) { const tris = resolve(id, null); if (tris) for (const tri of tris) emit(tri, null); }
    }
