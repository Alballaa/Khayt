...
async function fetchLibrary(token, opts) {
  ...
  if (Array.isArray(data)) return { items: data, count: data.length, filtered: false, syncedAt: null };
  throw new Error(
...
async function syncLibrary(token, opts) {
  ...
  if (state.syncedAt) {
    ...
    if (probe.filtered && probe.count === 0) {
      ...
      // change.
      return { items: state.items, syncedAt: state.syncedAt, unchanged: true, fetched: 1 };
    }
    const full = await fetchOne(token, { baseUrl: o.baseUrl });
    return { items: full.items, syncedAt: full.syncedAt || state.syncedAt, unchanged: false, fetched: 2 };
  }

  const full = await fetchOne(token, { baseUrl: o.baseUrl });
  return { items: full.items, syncedAt: full.syncedAt, unchanged: false, fetched: 1 };
}
...
module.exports = {
  explainLibraryStatus, syncLibrary, verifyChecksum,
