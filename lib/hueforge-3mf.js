...
(function (global) {
  ...
  function modelSettingsXml(name, tx, ty) {
  ...
  function layerRangesXml(bands, layerH) {
    ...
    const ranges = bands.map((b, i) =>
      ...
      + '   <option opt_key="layer_height">' + num(i === 0 ? baseLayerH : layerH) + '</option>\n'
      + '  </range>').join('\n');
    return '<?xml version="1.0" encoding="utf-8"?>\n<objects>\n <object id="1">\n' + ranges + '\n </object>\n</objects>\n';
  }
  ...
  function buildU1_3mf(o) {
    ...
    const members = [
      ...
      { name: 'Metadata/model_settings.config', data: modelSettingsXml(name, tx, ty) },
