...
async function downloadItem(item, destDir) {
  ...
  await assertSafeDownloadUrl(item.downloadUrl);
  ...
  // Cap the payload so one design file can't exhaust memory/disk. A server that lies about
  // content-length is already bounded by the HTTPS-origin allow-list in assertSafeDownloadUrl.
  const declared = Number(res.headers.get('content-length') || 0);
  if (declared > MAX_FILE_BYTES) throw new Error('"' + (item.title || item.slug) + '" is too large to download.');
  const buf = Buffer.from(await res.arrayBuffer());
  if (buf.length > MAX_FILE_BYTES) throw new Error('"' + (item.title || item.slug) + '" is too large to download.');
