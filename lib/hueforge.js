...
(function (global) {
  ...
  function heightfieldToMesh(solve, opts) {
    ...
    // top surface (up) + bottom surface (down), sharing X/Y grid coords
    for (let j = 0; j < H - 1; j++) {
      for (let i = 0; i < W - 1; i++) {
        const x0 = X(i), x1 = X(i + 1), y0 = Y(j), y1 = Y(j + 1);
        quad([x0, y0, vh(i, j)], [x1, y0, vh(i + 1, j)], [x1, y1, vh(i + 1, j + 1)], [x0, y1, vh(i, j + 1)]);      // top
        quad([x0, y0, 0], [x0, y1, 0], [x1, y1, 0], [x1, y0, 0]);                                                   // bottom (reversed)
      }
    }
    // skirt walls along the four borders (top edge → z=0), sharing the border grid vertices
    for (let i = 0; i < W - 1; i++) {
      const x0 = X(i), x1 = X(i + 1);
      quad([x0, Y(0), vh(i, 0)], [x1, Y(0), vh(i + 1, 0)], [x1, Y(0), 0], [x0, Y(0), 0]);                            // j=0
      quad([x1, Y(H - 1), vh(i + 1, H - 1)], [x0, Y(H - 1), vh(i, H - 1)], [x0, Y(H - 1), 0], [x1, Y(H - 1), 0]);    // j=H-1
    }
    for (let j = 0; j < H - 1; j++) {
      const y0 = Y(j), y1 = Y(j + 1);
      quad([X(0), y1, vh(0, j + 1)], [X(0), y0, vh(0, j)], [X(0), y0, 0], [X(0), y1, 0]);                            // i=0
      quad([X(W - 1), y0, vh(W - 1, j)], [X(W - 1), y1, vh(W - 1, j + 1)], [X(W - 1), y1, 0], [X(W - 1), y0, 0]);    // i=W-1
    }
