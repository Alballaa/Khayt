...
(function (global) {
  ...
  /**
   * Build a watertight "stepped column" solid from a solved heightfield, sized to the bed.
   * Each working pixel becomes a rectangular column of height k·layerH; top caps, a single
   * bottom plane, and vertical walls at every height step (and the border) close the solid.
   * (Stepped columns match how the print actually lays down — layer terraces — and are
   * guaranteed manifold; concept per Kromacut's heightfield mesher, reimplemented.)
  ...
  function heightfieldToMesh(solve, opts) {
    ...
    const sx = widthMm / W, sy = sx;              // square source pixels
    const zAt = (i, j) => (i < 0 || j < 0 || i >= W || j >= H) ? 0 : hts[j * W + i] * layerH;
    const tris = [];
    const quad = (a, b, c, d) => { tris.push([a, b, c]); tris.push([a, c, d]); };
    // Y is flipped so the image sits upright on the bed (+Y = image up).
    const X = (i) => i * sx;
    const Y = (j) => (H - j) * sy;
    ...
    for (let j = 0; j < H; j++) {
      for (let i = 0; i < W; i++) {
        const z = zAt(i, j);
        if (z > maxZ) maxZ = z;
        const x0 = X(i), x1 = X(i + 1), y0 = Y(j + 1), y1 = Y(j);
        // top cap (CCW seen from +Z)
        quad([x0, y0, z], [x1, y0, z], [x1, y1, z], [x0, y1, z]);
        // walls where this column is taller than a neighbour (taller side owns the face)
        const zr = zAt(i + 1, j), zl = zAt(i - 1, j), zt = zAt(i, j - 1), zb = zAt(i, j + 1);
        if (z > zr) quad([x1, y0, zr], [x1, y1, zr], [x1, y1, z], [x1, y0, z]);   // +X
        if (z > zl) quad([x0, y1, zl], [x0, y0, zl], [x0, y0, z], [x0, y1, z]);   // -X
        if (z > zt) quad([x0, y1, zt], [x1, y1, zt], [x1, y1, z], [x0, y1, z]);   // +Y (image up)
        if (z > zb) quad([x1, y0, zb], [x0, y0, zb], [x0, y0, z], [x1, y0, z]);   // -Y
      }
    }
    // single bottom plane over the whole footprint (CW seen from +Z → faces down)
    quad([0, 0, 0], [0, H * sy, 0], [W * sx, H * sy, 0], [W * sx, 0, 0]);
    return { triangles: tris, sizeMm: { x: W * sx, y: H * sy, z: maxZ }, triangleCount: tris.length };
  }
