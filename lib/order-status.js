...
(function (global) {
   ...
   *
   * `ctx`: `{ now, inventory, holdReason }`. Call `gate()` first — `apply()`
   * assumes the move has already been allowed, and a caller that skips the gate
   * has decided to skip it. `holdReason` is read only when moving to `on_hold`,
   * and only when the key is present at all.
   *
