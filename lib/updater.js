...
// Bed Ready ships from this same repo but on its own release lane (no publish
// target wired yet — see electron-builder.bedready.js `publish: null`). So it must
// NOT poll Khayt's GitHub feed, or it would offer a Khayt build as an "update".
const RELEASES_URL = isBedReady
  ? 'https://bedready.io/download'
...
function registerUpdater({ app, fs, ipcMain, BrowserWindow, encryptForDisk, dataFilePath, backupsDir }) {
  applyUpdateOptions({ allowBeta: false });
  ...
  function setupAutoUpdater(win) {
    ...
    // Bed Ready has no update feed baked in yet — skip the automatic poll so it
    // never hits Khayt's feed nor logs a "provider not configured" error on boot.
    if (app.isPackaged && !isBedReady) {
  ...
  ipcMain.handle('hub:check-for-updates', async () => {
    ...
    // Bed Ready: no published feed → don't invoke autoUpdater (it would throw a
    // "provider not configured" error). Report a clean, on-brand status instead.
    if (isBedReady) {
      return {
        status: 'not-available',
        currentVersion,
        latestVersion: currentVersion,
        message: `You're running Bed Ready ${currentVersion}. Visit ${RELEASES_URL} for the latest build.`,
      };
    }
    try {
