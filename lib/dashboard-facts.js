...
function lateOrderIds(attention, input) {
  ...
  try {
    ...
    // Attention is advisory: a board still renders without it. But unlike the
    // catch this replaces, nothing here is expected to throw, so a throw is a
    // defect rather than a supported path.
    return ids;
  }
  // `{count, items}` is the documented shape. An array is tolerated so a future
  // change to the engine degrades rather than silently empties every board.
...
function dashboardFacts(input) {
  ...
  const late = lateOrderIds(inp.attention, { machines, orders, statusCache: cache, now });
...
const api = { dashboardFacts, lateOrderIds, ACTIVE_STATUSES };
