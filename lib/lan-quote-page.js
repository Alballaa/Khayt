 ...
 */
function renderLanQuoteApprovalPage({ order, shopName, approvePath, alreadyApproved = false, expired = false }) {
    ...
    actionBlock = `
      ...
      <script>
        document.getElementById('approveBtn').addEventListener('click', async function() {
          ...
          try {
            const r = await fetch(${JSON.stringify(approvePath)}, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'approve' }) });
...
<div class="container">
  ...
  <div class="card">
    ...
    <tfoot><tr class="total-row"><td>Total</td><td style="text-align:right;">${price} SAR</td></tr></tfoot></table>
    ${actionBlock}
  </div>
</div></body></html>`;
}
