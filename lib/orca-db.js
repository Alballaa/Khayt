...
/** Locate a preset json across all vendor dirs for a given kind (machine/process/filament). */
function findPresetFile(kind, name) {
  const root = activeRoot();
  if (!root) return null;
  let vendors = [];
  try { vendors = fs.readdirSync(root); } catch (_) { return null; }
...
function listU1Processes() {
  ...
  const seen = new Set();
  let vendors = [];
  try { vendors = fs.readdirSync(root); } catch (_) { return []; }
  for (const vendor of vendors) {
