...
/**
 * Compression of the store payload, and the two-release rollout it requires.
 *
 * Sync is blob-first: push() sends the WHOLE store every time, base64'd inside a
 * JSON body and — until now — uncompressed. Measured on a real store, 55,593
 * bytes of plaintext went out as 74,124 bytes where gzip would have sent 7,556.
 * Bandwidth is the binding cost of running Khayt Cloud
 * (docs/KHAYT-CLOUD-COST-PER-SHOP.md), so this is the cheapest large saving
 * available.
 *
 * `COMPRESS_ON_WRITE` IS DELIBERATELY FALSE, and flipping it is a separate
 * release. The note above explains why: nothing reads `blob.v`, so a client that
 * predates this change ignores `z` too, JSON.parses gzip bytes, and throws on
 * every pull. It would not lose data — the optimistic `baseRev` guard means a
 * client that cannot pull cannot push over the top — but it would sit in a sync
 * error loop until it updated, which is not a thing to do to a shop with two
 * devices mid-rollout.
 *
 * So: this release teaches every client to READ both shapes. A later one flips
 * this constant to true, once readers are in the field. Same sequencing as the
 * portal read gate, and for the same reason — the blocker is adoption, not code.
 */
const STORE_COMPRESSION = 'gzip';
const COMPRESS_ON_WRITE = false;
