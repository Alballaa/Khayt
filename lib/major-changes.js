...
(function (global) {
  ...
  function parseMajorChanges(notes) {
    ...
    for (let j = i + 1; j < lines.length; j++) {
      ...
      if (bullet) { const v = plain(bullet[1]); if (v) items.push(v); continue; }
      // A wrapped continuation belongs to the bullet above it, not to nothing.
      if (items.length && /^\s{2,}\S/.test(line)) {
        items[items.length - 1] = plain(items[items.length - 1] + ' ' + line);
      }
    }
    return { needsConsent: items.length > 0, items };
  }
