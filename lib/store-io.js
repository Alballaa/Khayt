...
function createStoreIo({ app, fs, safeStorage, safeJsonParse, crypto, onStoreUpdated }) {
  ...
  function encryptForDisk(data) {
    ...
    // The Drive refresh token is a long-lived key to the shop's Google account.
    // It outranks the bucket secret: a leaked bucket key reaches one bucket, this
    // reaches every file Khayt ever put in their Drive, and it does not expire.
    for (const k of ['refreshToken', 'clientSecret'])
      if (d?.settings?.printLibrary?.gdrive?.[k])
        d.settings.printLibrary.gdrive[k] = encryptStoreField(d.settings.printLibrary.gdrive[k]);
  ...
  function hasPlaintextSecrets(data) {
    ...
    for (const k of ['refreshToken', 'clientSecret']) if (needs(s.printLibrary?.gdrive?.[k])) return true;
  ...
  function decryptStoreSecrets(data) {
    ...
    for (const k of ['refreshToken', 'clientSecret'])
      if (data?.settings?.printLibrary?.gdrive?.[k])
        data.settings.printLibrary.gdrive[k] = decryptStoreField(data.settings.printLibrary.gdrive[k]);
    if (Array.isArray(data?.machines)) {
  ...
  function maskStoreSecretsForRenderer(data) {
    ...
    mask(data.settings?.printLibrary?.gdrive, 'refreshToken');
    mask(data.settings?.printLibrary?.gdrive, 'clientSecret');
    if (Array.isArray(data?.machines)) {
      data.machines = data.machines.map((m) => {
        if (!m?.printerApi) return m;
        const pa = { ...m.printerApi };
...
module.exports = { createStoreIo, STORE_SECRET_MASK, MAX_STORE_BYTES };
