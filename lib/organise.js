...
(function (global) {
   ...
   *
   * `group` first, `folder` second. A record written before groups existed has
   * only `folder`; one written since has both and they agree.
   */
  const groupOf = (rec) => normalise(rec && (rec.group != null && rec.group !== '' ? rec.group : rec.folder));
  ...
  const api = { MAX, normalise, unify, counts, known, assign, groupOf, categoryOf, isIn, membersOf, nameKey: key };
