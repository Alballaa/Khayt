        ...
        // ── Quote approval ──────────────────────────────────────
          ...
          req.on('end', async () => {
            try {
              ...
              await persistLanStoreUpdate(storeData);
              if (getMainWindow() && !getMainWindow().isDestroyed()) {
                getMainWindow().webContents.send('lan-order-updated', { id: safeId, status: 'pending', clientApprovedAt: storeData.printLog[idx].clientApprovedAt });
              }
              const projectName = lanEscapeHtml(order.project || order.id);
              res.setHeader('Content-Type', 'text/html; charset=utf-8');
              res.writeHead(200);
...
module.exports = { registerLanServer, lanEscapeHtml, safeTokenEqual };
