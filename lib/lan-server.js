...
ipcMain.handle('hub:start-tunnel', async (_e, arg) => {
  ...
  // Warn (do NOT block) when the PIN is too weak for public internet exposure.
  const pinWarning = weakTunnelPinWarning(lanPin, true);
  if (pinWarning) console.warn('[lan-server]', pinWarning);
  ...
  try {
    ...
    return { ok: true, url: tunnel.url, ...(pinWarning ? { warning: pinWarning } : {}) };
  } catch (e) {
...
setTimeout(()=>location.reload(),30000);
        ...
        // ── Zid inbound order webhook ───────────────────────────
          ...
          res.writeHead(404);
          res.end(JSON.stringify({ error: 'Not found', endpoints: ['/api/status','/api/orders','/api/queue','/api/machines','/api/inventory','/api/waiting-list','/api/clients','/api/webhook/printer/:machineId','/calendar.ics','/intake','/api/intake','/api/webhook/salla','/api/webhook/zid'] }));
        }
      });
