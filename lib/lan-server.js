    ...
    try {
      lanServer = http.createServer((req, res) => {
        ...
        // ── iOS companion: inventory ────────────────────────────
          ...
          req.on('end', async () => {
            try {
              const spool = parseLanJsonBody(body);
              if (!spool || typeof spool !== 'object' || Array.isArray(spool)) {
          ...
          if (!order) {
            ...
            (async () => {
              try {
                ...
                const ord = storeData.printLog[idx];
                const hadToken = !!ord.quoteApprovalToken;
                const approvalToken = ensureQuoteApprovalToken(ord);
                if (!hadToken) await persistLanStoreUpdate(storeData);
                const alreadyApproved = ord.status !== 'quote' && !(ord.status === 'on_hold' && ord.hasQuote);
                res.writeHead(200);
                res.end(renderLanQuoteApprovalPage({
                  ...
                  approvalToken,
                  alreadyApproved,
                  expired: !alreadyApproved && isQuoteExpired(ord),
                  currencyLabel: store.settings?.currency || 'SAR',
                }));
              } catch (e) {
                res.writeHead(500, { 'Content-Type': 'text/plain; charset=utf-8' });
                res.end('Error loading quote');
              }
            })();
          }
...
setTimeout(()=>location.reload(),30000);
        ...
        // ── iCal feed ───────────────────────────────────────────
          ...
          if (!pin) {
            ...
            res.end('Calendar feed requires a LAN PIN in Khayt Settings → LAN API.');
            return;
          }
        ...
        // ── Online intake form ──────────────────────────────────
          ...
          const shopName = lanEscapeHtml(store.settings?.shopName || 'Khayt');
          res.setHeader('Cache-Control', 'no-cache');
          const onlineOpen = !!store.settings?.onlineEnabled;
...
module.exports = { registerLanServer, lanEscapeHtml, safeTokenEqual, normalizePrinterEvent, pickLanIPv4 };
