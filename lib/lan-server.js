        ...
        // either. Both are defaults rather than something remembered here.
          ...
          req.on('end', async () => {
            try {
              ...
              await updateStoreOnDisk((cur) => ({ ...cur, inventory: [...(cur.inventory || []), spool] }));
          ...
          req.on('end', async () => {
            try {
              ...
              const spool = { ...view[viewIdx], remaining: grams, weightRemaining: grams };
              ...
              await updateStoreOnDisk((cur) => {
                ...
                if (i === -1) return cur;
                inv[i] = { ...inv[i], remaining: grams, weightRemaining: grams };
