  ...
  return new Promise(resolve => {
    try {
      lanServer = http.createServer((req, res) => {
        ...
        if (!apiTokenRec && !isAlwaysPublic && !isIntakePublicGet && !isIntakeSessionPost && !isIntakeSubmitPost && (requirePin || (pin && !isSurveyEndpoint))) {
        ...
        const checkPinForGet = () => {
          // A valid scoped API token already authenticated this request and had its scope
          // enforced above, so it satisfies this per-route gate too (tokens are the
          // automation equivalent of the owner PIN).
          if (apiTokenRec) return true;
          if (!pin) {
