...
(function (global) {
   ...
   */
  function bestPhysicalSet(hexes, usage, pinned) {
    usage = usage || []; pinned = pinned || [];
    var all = hexes.map(function (_, i) { return i; });
    if (all.length <= 4) return all.sort(function (a, b) { return (usage[b] || 0) - (usage[a] || 0); });
  ...
  function planFullSpectrum(colors, usage, opts) {
    ...
    var physOld = (opts.physical && opts.physical.length === 4)
      ? opts.physical.slice(0, 4)
      : bestPhysicalSet(colors.slice(0, n), usage, opts.pinned || []);
