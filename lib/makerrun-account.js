...
async function getAccessToken(userDataDir, now = Math.floor(Date.now() / 1000)) {
  ...
  if (!d || !d.refresh) throw new Error('Not linked. Connect the app from makerrun.com/app-link.');
...
async function doRefresh(userDataDir, usedRefresh, now) {
  ...
  if (res.status === 401) {
    ...
    throw new Error('Your MakerRun link expired — reconnect from makerrun.com/app-link.');
  }
