...
(function (global) {
  ...
  function renderStlThumbnail(triangles, opts) {
    ...
    // Camera: look from front-right-top (Z is up for print STLs).
    const camDir = norm([1, -1, 0.75]);        // object → camera (also the light)
    ...
    // Project every vertex; track screen-space bounds.
    ...
    for (const f of faces) {
      const c = f.shade;
      ctx.fillStyle = `rgb(${Math.round(r * c)},${Math.round(g * c)},${Math.round(b * c)})`;
      ctx.beginPath();
