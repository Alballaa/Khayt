...
- Split from `app.js`: `util.js`, `currency.js`, `calculator-cost.js`, `build.js` (calculator tab, quote cart, presets, quote templates), `inventory.js` (spools, catalog, POs, NFC import), `kanban.js`, `invoicing.js`, `logs.js`, `settings.js`, `order-flows.js`, `waiting-list.js`, `dashboard.js`, `analytics.js` (charts + retention, capacity, break-even, export report; with unit tests).
