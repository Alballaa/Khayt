#!/usr/bin/env node
/**
 * Dev helper: launch the app with a given design theme + demo data and
 * screenshot one or more tabs. Usage:
 *   node scripts/shot-theme.mjs <themeId> [tab ...] [--dark]
 * e.g. node scripts/shot-theme.mjs command dashboard-tab --dark
 * Tabs default to the full set. Output: .wb-shots/<theme>-<tab>[-dark].png
 */
...
const theme = positional[0] || 'workbench';
const tabs = positional.slice(1);
...
try {
  ...
  demoStore.settings.designTheme = theme;
  ...
  }, { id: theme, appearance: dark ? 'dark' : 'light' });

  await page.waitForFunction(
    (id) => document.body.classList.contains(`khayt-${id}`)
  ...
  for (const tab of TABS) {
    ...
    const out = path.join(outDir, `${theme}-${tab}${dark ? '-dark' : ''}.png`);
